...
export async function middleware(request: NextRequest) {
  ...
  if (isAdminRoute(pathname) || isAdminApiRoute(pathname)) {
    ...
    const tokenRoles = Array.isArray(token.roles) ? token.roles : [token.role];
    const isAdmin = tokenRoles.includes("ADMIN") || tokenRoles.includes("SUPER_ADMIN");

    if (!isAdmin) {
      console.warn(`[MIDDLEWARE] Unauthorized admin access attempt: ${token.email} tried to access ${pathname}`);
