...
export async function GET(request: Request) {
  try {
    ...
    const transformedCustomers = customers.map((customer: CustomerWithDetails) => {
      const totalSpent = customer.orders.reduce((sum: number, order: typeof customer.orders[number]) => sum + order.totalInCents, 0);
      const lastOrder = customer.orders.length > 0 ? customer.orders[0] : null;
      const roleNames = customer.roles.map((entry: typeof customer.roles[number]) => entry.role.name);
