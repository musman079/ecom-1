  ...
  return (
    <div className="min-h-screen bg-[#070d17] text-[#eaf2ff]">
      ...
      <section ref={heroRef} className="relative flex min-h-[60vh] items-end overflow-hidden px-6 pb-16 pt-32 lg:px-16">
        ...
        />
        <motion.div
          className="pointer-events-none absolute -right-20 top-1/3 h-[400px] w-[400px] rounded-full bg-[#65f3de]/10 blur-[120px]"
          animate={{ opacity: [0.3, 0.6, 0.3], scale: [1, 1.1, 1] }}
          transition={{ duration: 8, repeat: Infinity, ease: "easeInOut" }}
        />
        <motion.div
          className="pointer-events-none absolute -left-16 bottom-0 h-[350px] w-[350px] rounded-full bg-[#497cff]/12 blur-[100px]"
        ...
        <div className="relative mx-auto w-full max-w-[1400px]">
          <FadeIn>
            <p className="text-[10px] font-bold uppercase tracking-[0.3em] text-[#65f3de]">Our Story</p>
            <h1 className="mt-3 text-5xl font-black uppercase tracking-[-0.05em] sm:text-6xl md:text-7xl">
              About
              <br />
              <span className="bg-gradient-to-r from-[#65f3de] to-[#4f8cff] bg-clip-text text-transparent">USOLSTICE</span>
            </h1>
        ...
        <Stagger className="mx-auto grid w-full max-w-[1400px] grid-cols-2 gap-px md:grid-cols-4">
          {stats.map((stat) => (
            <StaggerItem key={stat.label} className="flex flex-col items-center gap-2 px-6 py-12 text-center">
              <span className="text-3xl font-black tracking-tight text-[#65f3de] sm:text-4xl">
      ...
      <main className="mx-auto w-full max-w-[1400px] px-6 xl:px-12">
        ...
        <section className="py-24">
          <FadeIn>
            <p className="text-[10px] font-bold uppercase tracking-[0.3em] text-[#65f3de]">What We Stand For</p>
          ...
          <Stagger className="mt-14 grid gap-8 sm:grid-cols-2 lg:grid-cols-4">
            {values.map((val) => (
              <StaggerItem key={val.title}>
                <div className="group h-full rounded-2xl border border-white/10 bg-white/[0.03] p-8 backdrop-blur-sm transition hover:border-[#65f3de]/20 hover:bg-white/[0.05]">
                  <div className="flex h-14 w-14 items-center justify-center rounded-xl bg-gradient-to-br from-[#65f3de]/15 to-[#4f8cff]/15 transition group-hover:from-[#65f3de]/25 group-hover:to-[#4f8cff]/25">
                    <span className="material-symbols-outlined text-2xl text-[#65f3de]">{val.icon}</span>
                  </div>
        ...
        <section className="pb-24">
          <FadeIn>
            <p className="text-[10px] font-bold uppercase tracking-[0.3em] text-[#65f3de]">Our Journey</p>
            <h2 className="mt-3 text-4xl font-black uppercase tracking-[-0.04em]">Milestones</h2>
          </FadeIn>

          <div className="relative mt-14">
            {/* Vertical line */}
            <div className="absolute left-6 top-0 h-full w-px bg-gradient-to-b from-[#65f3de]/40 via-[#4f8cff]/30 to-transparent md:left-1/2" />
            ...
            {milestones.map((item, idx) => (
              <FadeIn key={item.year} delay={idx * 0.08}>
                <div className={`relative mb-12 flex items-start gap-8 pl-16 md:pl-0 ${idx % 2 === 0 ? "md:flex-row" : "md:flex-row-reverse"}`}>
                  ...
                  {/* Dot */}
                  <div className="absolute left-4 top-1 flex h-5 w-5 items-center justify-center rounded-full border-2 border-[#65f3de] bg-[#070d17] md:left-1/2 md:-translate-x-1/2">
                    <div className="h-2 w-2 rounded-full bg-[#65f3de]" />
                  </div>
        ...
        {/* CTA */}
        <FadeIn>
          <section className="mb-24 rounded-3xl border border-white/10 bg-gradient-to-br from-[#65f3de]/5 to-[#4f8cff]/5 p-12 text-center backdrop-blur-sm lg:p-20">
            <h3 className="text-3xl font-black uppercase tracking-[-0.04em] sm:text-4xl">
              Join the <span className="text-[#65f3de]">USOLSTICE</span> Movement
            </h3>
